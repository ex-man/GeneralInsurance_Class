...
Do you know what this is? It is the core of Reserving and in many cases also pricing. This is also how the losses from previous class were produced. It is a very simple way to get many answers like:
...
*Think about some math, that could project the future for the "unfinished lines" (e.g. liner models?)* To explore a bit of math, look at the examples from help section on "chainladder" function
...
weighted.mean(y / x, x)
lm(y ~ x + 0,  weights = 1 / x)$coefficients

summary(chainladder(RAA))
```

*Now try to predict what happens with the line using the chain ladder technique (hint: use predict() function). You can also try producing plots for predicted values (now two triangles that form a matrix).*

The thing that is actually predicting the future is the age-to-age factors. Can you find them in the chainladder object?
...
# inspect the object and get final age-to-age factors
str(chainladder(RAA), max.level = 1)
str(chainladder(RAA)$Models[[1]], max.level = 2)

# your code
chainladder(RAA)$Models[[1]]$coefficients
chainladder(RAA)$Models[[2]]$coefficients
```
...
Now let's have a look at a couple of different triangles. The data is provided for 2 different businesses. It also shows 2 different claim types for each. Can you describe how different they are? Hint: Consider the length of tail, volatility, average sizes... Hint2: There are 2 types of data - paid and case. Start using Paid...

Load data with loss development information

```{r}
dt_PaidCase <- read.csv("./Data/lesson6_PaidCase.csv")
...
*YOUR SOLUTION* understand the data =\> run some simple overviews
...
*STEP 1: Take paid data for House business and Small claim size*
...
*STEP 5: Get the volume weighted averages (vwtd) and multiply, to get the ultimate aging factor (from year 1 to year 10)* HINT1: You can use str() function to get a hint about potential extraction of 'vwtd' HINT2: attr() can be used to extract attributes from an object..
...
dt_LatestView <- read.csv("./Data/lesson6_latestView.csv") %>% 
...
```{r}
NPV_data <- read.csv("./Data/lesson6_NPV.csv")
```
