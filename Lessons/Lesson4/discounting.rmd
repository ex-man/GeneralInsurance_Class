...
```{r}
dt_KPI <- read.csv("C:/Users/Lucia_Sebestova/Documents/Matfyz(4)/VTvAktuarstve/GeneralInsurance_Class/Data/lesson2_KPI.csv")
dt_LatestView <- read.csv("C:/Users/Lucia_Sebestova/Documents/Matfyz(4)/VTvAktuarstve/GeneralInsurance_Class/Data/lesson4_latestView.csv")
dt_PaidCase <- read.csv("C:/Users/Lucia_Sebestova/Documents/Matfyz(4)/VTvAktuarstve/GeneralInsurance_Class/Data/lesson4_PaidCase.csv")
```
...
```{r}
Paid_HH_Large<-dt_PaidCase %>% filter( Business == "House" & ClaimSize == "Large" & dataset_type == "PAID")  
head(Paid_HH_Large)
Paid_HH_Large_triangle <- Paid_HH_Large %>% as.triangle(origin = "ay", dev = "dy", "SumOfamount")
Paid_HH_Large_triangle
plot(Paid_HH_Large_triangle) #pre udalosti, ktore vznikli v roku 2008 (2) nam vyslo, ze o 2 roky (a o viac rokov) je vysie Sumofamount ("vyplatenie celkovych narokov (pohladavok)- celkovu hodnotu, ktoru musia vyplatit") ako vyslo pre udalosti vzniknute v roku 2007 (1), pri ktorych sme zistili aka je vyska Sumofamount o 3 roky (a viac rokov). Najvacsie zmeny v Sumofamount nastavaju o rok a o 2 roky po vzniku udalosti, potom sa Sumofamount zacina ustalovat. Najviac ma Sumofamount pre rok pre 2011 (5) a najmenej pre rok 2014 (8)   
plot(predict(chainladder(Paid_HH_Large_triangle))) # tento uz obsahuje aj ocakavane hodnoty 
ata(Paid_HH_Large_triangle)

Paid_3rd_Large<-dt_PaidCase %>% filter( Business == "3rd Party" & ClaimSize == "Large" & dataset_type == "PAID")
head(Paid_3rd_Large)
Paid_3rd_Large_triangle <- Paid_3rd_Large %>% as.triangle(origin = "ay", dev = "dy", "SumOfamount")
Paid_3rd_Large_triangle
plot(Paid_3rd_Large_triangle) 
plot(predict(chainladder(Paid_3rd_Large_triangle)))
predict(chainladder(Paid_3rd_Large_triangle))#chcela som vidiet hodnoty ake ocakava v roku 2016 (teda aku hodnotu ocakava napr o rok, to je v roku 2017), lebo dost ovplivnili graf "predict(chainladder(Paid_3rd_Large_triangle))" a je vidno ze odhaduju dost vysoke hodnoty Sumofamount
ata(Paid_3rd_Large_triangle)

Paid_3rd_Small<-dt_PaidCase %>% filter( Business == "3rd Party" & ClaimSize == "Small" & dataset_type == "PAID")
head(Paid_3rd_Small)
Paid_3rd_Small_triangle<- Paid_3rd_Small %>% as.triangle(origin = "ay", dev = "dy", "SumOfamount")
Paid_3rd_Small_triangle
plot(Paid_3rd_Small_triangle) 
plot(predict(chainladder(Paid_3rd_Small_triangle)))
ata(Paid_3rd_Small_triangle)
```
...
```{r}
dt_PC<-dt_PaidCase%>% mutate(dataset_type="PC")
head(dt_PC)
PC_HH_Large<-dt_PC %>% filter( Business == "House" & ClaimSize == "Large" )  
head(PC_HH_Large)
PC_HH_Large_triangle <- PC_HH_Large %>% as.triangle(origin = "ay", dev = "dy", "SumOfamount")
PC_HH_Large_triangle
plot(PC_HH_Large_triangle) 
plot(predict(chainladder(PC_HH_Large_triangle)))
ata(PC_HH_Large_triangle)

PC_HH_Small<-dt_PC %>% filter( Business == "House" & ClaimSize == "Small" )  
head(PC_HH_Small)
PC_HH_Small_triangle <- PC_HH_Small %>% as.triangle(origin = "ay", dev = "dy", "SumOfamount")
PC_HH_Small_triangle
plot(PC_HH_Small_triangle) 
plot(predict(chainladder(PC_HH_Small_triangle)))
ata(PC_HH_Small_triangle)

PC_3rd_Large<-dt_PC %>% filter( Business == "3rd Party" & ClaimSize == "Large" )  
head(PC_3rd_Large)
PC_3rd_Large_triangle <- PC_3rd_Large %>% as.triangle(origin = "ay", dev = "dy", "SumOfamount")
PC_3rd_Large_triangle
plot(PC_3rd_Large_triangle) 
plot(predict(chainladder(PC_3rd_Large_triangle)))
ata(PC_3rd_Large_triangle)

PC_3rd_Small<-dt_PC %>% filter( Business == "3rd Party" & ClaimSize == "Small" )  
head(PC_3rd_Small)
PC_3rd_Small_triangle <- PC_3rd_Small %>% as.triangle(origin = "ay", dev = "dy", "SumOfamount")
PC_3rd_Small_triangle
plot(PC_3rd_Small_triangle) 
plot(predict(chainladder(PC_3rd_Small_triangle)))
ata(PC_3rd_Small_triangle)
