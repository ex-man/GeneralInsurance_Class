...
```{r}
dt_Policy <- read.csv("/cloud/project/Data/lesson4_PolicyHistory.csv")
...
```{r}
dt_Claims <- read.csv("/cloud/project/Data/lesson4_Claims.csv")
...
dt_Claims %>% nrow()
dt_Claims %>%
  distinct(NrPolicy, NrObject, ClmYr) %>%
  nrow()
```
...
To join one table to another based on some variable(s), we can use a `___join()` functions. There are multiple types of joins. 

![joins](/cloud/project/Support/join_diagram.png)

- left join(): keeps all observations in x 
- inner join(): keeps observations from x that have a matching key in y (intersection between datasets) 
- full_join(): keeps all observations in x and y
− semi_join(): keeps observations from x that have a matching key in y and never duplicate rows of x (intersection between datasets)
− anti_join(): returns all rows from x without a match in y

Since not every policy will have a claim, we will be using left join to preserve policy information.
...
```{r}
saveRDS(dt_pol_w_claims, "/cloud/project/Data/dt_pol_w_claims.rds")
```