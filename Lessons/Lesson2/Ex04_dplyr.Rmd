...
dt_KPI %>%
  select(all_of(region_col), Losses) %>%
  head()
```
...
> Or we may want to `summarise` data to see some basic statistic about it. Let's summarize the tree basic insurance metrics we talked about.

```{r}
dt_KPI %>%
  summarise(
    Total_Premium = sum(Premium, na.rm = TRUE),
    Average_Losses = mean(Losses, na.rm = TRUE),
    Maximum_Expenses = max(Expenses, na.rm = TRUE)
  )
```

> We can see the summary split by one (or more) of columns using the 'group_by' function. This is useful when comparing different categories, e.g. Regions.
